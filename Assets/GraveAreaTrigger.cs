...
public class GraveAreaTrigger : MonoBehaviour
{
    [SerializeField] private TextMeshProUGUI messageText; // Reference to the UI TextMeshProUGUI component for the message
    private bool isPlayerNear = false; // Flag to track if the player is near the GraveArea
    ...
    void OnTriggerEnter2D(Collider2D collider)
    {
        ...
        if (collider.CompareTag("Player"))
        {
            ...
            if (messageText != null)
            {
                messageText.text = "Leave flowers: Tap F"; // Set the message to "Leave flowers"
